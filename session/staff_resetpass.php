...
if (!isset($_GET['resettoken'])) {
    ...
    header('Location: ../authentication/stafflogin.php'); // Redirect to login
    exit;
}
...
$stmt = $conn->prepare("SELECT staff_id FROM staff WHERE ResetToken = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($newPassword !== $confirmPassword) {
        ...
        header('Location: staff_resetpass.php?resettoken=' . $resettoken); // Retain token in URL
        exit;
    }
    ...
    $stmt = $conn->prepare("UPDATE staff SET password = ?, ResetToken = NULL WHERE staff_id = ?");
...
<section class="flex items-center justify-center min-h-screen">
  <div class="w-full max-w-md px-4 sm:px-6 p-6 bg-white rounded-lg shadow dark:border md:mt-0 dark:bg-gray-800 dark:border-gray-700 sm:p-8">
      ...
      <form class="mt-4 space-y-4 lg:mt-5 md:space-y-5" action="staff_resetpass.php?resettoken=<?php echo $resettoken; ?>" method="POST">
      <div>
        <label class="text-gray-900 dark:text-white text-sm mb-2 block">New Password</label>
          ...
          Go Back to <a href="../authentication/stafflogin.php" class="font-medium text-blue-600 hover:underline dark:text-blue-500">Login</a>
          </p>
      </form>
  </div>
</section>
